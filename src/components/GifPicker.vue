        ...
        <div class="overflow-auto relative p-4 h-full w-full">
          ...
          <div v-else-if="tags && tags.length" class="grid grid-cols-2 grid-flow-row auto-rows-auto	gap-4">
            <div
                v-for="(tag, t) in tags"
                :key="t"
                class="h-20 rounded-lg bg-cover text-white flex items-center justify-center relative font-semibold font-xl border-2 border-transparent hover:border-blue transition duration-300 cursor-pointer group z-1 overflow-hidden "
...
export default {
  ...
  mounted () {
    fetch(`https://tenor.googleapis.com/v2/categories?key=${this.apiKey}`)
        .then(res => res.json())
  ...
  methods: {
    ...
    get (query, key) {
      fetch(`https://tenor.googleapis.com/v2/${query}&key=${this.apiKey}`)
          .then(res => res.json())
